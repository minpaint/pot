...
Кнопки общего уровня вверху страницы остаются:
...
## Известные причины, почему «не начинается скачивание»
1) JS отменил отправку формы:
   - Если кнопка верхнего уровня — нужны отмеченные сотрудники.
...
3) Ответ вернулся не как файл:
   - Проверить в `Network` → `Response` (в браузере) — не HTML ли?
